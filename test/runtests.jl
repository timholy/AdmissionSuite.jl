...
@testset "AdmissionsSimulation.jl" begin
    ...
    past_applicants = [(program=:NS, rank=7, offerdate=Date("2021-01-13"), decidedate=Date("2021-03-26"), decision=:y),
                       (program=:NS, rank=3, offerdate=Date("2021-01-13"), decidedate=Date("2021-04-15"), decision=:n),
                       (program=:CB, rank=6, offerdate=Date("2021-03-25"), decidedate=Date("2021-04-15"), decision=:y),
    ]

    applicant = (program=:NS, rank=11, offerdate=Date("2021-01-13"))

    fmatch = match_function((matchprogram=false, σs=0.01, σd=Inf); program_history)
    ...
    @test sc[end-1].program == :NS && sc[end-1].rank == 7

    fmatch = match_function((matchprogram=false, σs=Inf, σd=0.3); program_history)
    clike = match_clikelihood(fmatch, past_applicants, applicant, Date("2021-01-13"); program_history)
    like = diff(pushfirst!(copy(clike), 0))
    @test like[1] == like[2] == 1
    @test like[1] > like[3]

    fmatch = match_function((matchprogram=true, σs=0.01, σd=Inf); program_history)
    clike = match_clikelihood(fmatch, past_applicants, applicant, Date("2021-01-13"); program_history)
    like = diff(pushfirst!(copy(clike), 0))
    @test like[1] > like[2]
    @test like[3] == 0
