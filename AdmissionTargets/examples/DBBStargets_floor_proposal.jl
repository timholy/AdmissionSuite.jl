...
tgtshft = AdmissionTargets.targets_linear(program_napplicants, program_nfacultyt, nslots, 3)
tgtmin, tgtminparams = AdmissionTargets.targets_min(program_napplicants, program_nfaculty, nslots, 4)
...
slots2021 = [program_history[(program=prog, season=this_season)].target for prog in pnames]
...
push!(dfCmteB, ["% slots in 2021",
                aspct(program_history[(program="NS", season=2021)].target/nslots0),
                aspct(sum(program_history[(program=prog, season=2021)].target for prog in CmteB)/nslots0)])
