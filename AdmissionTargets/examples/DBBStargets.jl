...
function compute_schemes(program_data, facrecs)
    N = sum(pr->pr.second.target, program_data)  # total number of target slots across all programs
    program_napplicants = Dict(pk.program => pd.napplicants for (pk, pd) in program_data)
    program_nslots = Dict(pk.program => pd.target for (pk, pd) in program_data)
...
function aggregate(program_history::AbstractDict{PK,PD}, mergepairs)
    ph = empty(program_history)
    for (pk, pd) in program_history
        pksubs = (program=AdmissionTargets.substitute(pk.program, mergepairs), season=pk.season)
        if !haskey(ph, pksubs)
            ph[pksubs] = pd
        else
            pdprev = ph[pksubs]
            ph[pksubs] = (napplicants=pd.napplicants+pdprev.napplicants, target=pd.target+pdprev.target)
        end
    end
    return ph
end
...
for subst in (["CSB"=>"CmteB", "DRSCB"=>"CmteB", "HSG"=>"CmteB", "MGG"=>"CmteB"],  # CommitteeB programs
    ...
    AdmissionTargets.addprogram(name)
    try
        ...
        pd = aggregate(this_season, subst)
        fr = AdmissionTargets.aggregate(facrecs, subst)
        ...
        AdmissionTargets.delprogram(name)
    end
end
...
N = sum(pr->pr.second.target, this_season)
...
tgtsl = AdmissionTargets.targets_linear(program_napplicants, program_nfaculty, N, 3)  # linear floor (what we did in 2021)
