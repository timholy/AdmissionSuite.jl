...
function calibrate_service(progsvc::ListPairs{<:AbstractString,Service}, yrthresh = year(today())-7)
    progrange = merge_program_range!(copy(program_range), program_substitutions)
...
function faculty_effort(facrecs::ListPairs{<:AbstractString,FacultyRecord},
                        ...
                        sc=nothing,
                        progyears=merge_program_range!(copy(program_range), program_substitutions),
...
```math
n_i = \\sqrt{n_0^2 + \\frac{{N^\\prime}^2 w_i^2}{W^2}}.
```
...
`wᵢ` is the weight assigned to the `i`th program and is extracted from `program_nfaculty`,
and `W² = ∑ᵢ wᵢ^2`.
Very small programs might receive most of their slots from `n₀`,
whereas large programs receive approximately a fraction `N′/N < 1` of the slots they would otherwise receive
without `minslots`.

This "hyperbolic" approach provides graded program-leveling, reserving the largest boost for small programs.
It does not have sharp cutoffs: even programs that would have received barely more than `minslots` still get
some boost.
"""
